<template>
    <div>
              ...
              <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
                ...
                <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-white px-6 pb-2">
                  <div class="flex h-16 shrink-0 items-center">
                    <img class="h-8 w-auto" src="/logo.png.jpg" alt="StorifyAI" />
                    <p class="font-bold ml-2">StorifyAI</p>
                  </div>
      ...
      <div class="hidden lg:fixed lg:inset-y-0 lg:z-9 lg:flex lg:w-72 lg:flex-col">
        <!-- Sidebar component, swap this element with another sidebar if you like -->
          ...
          <nav class="flex flex-1 flex-col">
            <ul role="list" class="flex flex-1 flex-col gap-y-7">
              <li>
                <ul role="list" class="-mx-2 space-y-1">
                  <li v-for="item in navigation" :key="item.name">
                    <a :href="item.href" :class="[item.current ? 'bg-gray-50 text-indigo-600' : 'text-gray-900 hover:bg-gray-50 hover:text-indigo-600', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                      <component :is="item.icon" :class="[item.current ? 'text-indigo-600' : 'text-gray-400 group-hover:text-indigo-600', 'size-6 shrink-0']" aria-hidden="true" />
                      {{ item.name }}
                    </a>
                  </li>
                  <li>
                    <ul role="list" class="flex flex-1 flex-col gap-y-4 ml-8">
                      <li>
                        <a href="#" class="text-gray-900 hover:bg-gray-50 hover:text-indigo-600'">
                          No project yet
                        </a>
                      </li>
                    </ul>
              ...
              <li class="-mx-6 mt-auto">
                <a href="#" class="flex items-center gap-x-4 px-6 py-3 text-sm/6 font-semibold text-gray-900 hover:bg-gray-50">
                  <img class="size-8 rounded-full bg-gray-50" src="/emptyprofile.jpg" alt="" />
      ...
      <div class="sticky top-0 z-9 flex items-center gap-x-6 bg-white px-4 py-4 shadow-sm sm:px-6 lg:hidden">
      ...
      <Modal @close="uiStore.wlModalState = false"/>
    </div>
  </template>
